...
ValidarSupuestos <- function (respuesta ,modelo,confianza){
  ...
  if((shapiro > confianza) & (lillie > confianza)){
    
    print("Existe normaliadad y blablabla")
    
  }else{
    
    print("No Existe normaliadad y blablabla")
  }
  
  # Test de algo jummm
  
  baruch <- bptest(modelo) ### Test Breusch-Pagan valor.p =0.39
  baruch <- baruch$p.value

  golfred <- gqtest(modelo)
  golfred <- golfred$p.value

  if((baruch > confianza) & (golfred > confianza)){
    
    print("Existe homocet y blablabla")
    
  }else{
    
    print("No Existe homocet y blablabla")
  }
  
  indepenencia <- dwtest(STATEAID ~ modelo$residuals)
  indepenencia <- indepenencia$p.value
  
  if(indepenencia > confianza){
